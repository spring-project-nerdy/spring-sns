...
public class PostControllerTest {
  ...
  @Test
  @WithMockUser
  void 포스트삭제시_작성자와_삭제요청자가_다를경우_에러발생() throws Exception {
  ...
  @Test
  @WithMockUser
  void 포스트삭제시_삭제하려는_포스트가_존재하지_않을경우_에러발생() throws Exception {
  ...
  @Test
  @WithAnonymousUser
  void 피드요청시_로그인하지_않은경우_에러발생() throws Exception {
    mockMvc.perform(get("/api/v1/posts")
            .contentType(MediaType.APPLICATION_JSON)
        ).andDo(print())
        .andExpect(status().isUnauthorized());
  ...
  @Test
  @WithAnonymousUser
  void 내피드요청시_로그인하지_않은경우_에러발생() throws Exception {
    mockMvc.perform(get("/api/v1/posts/my")
            .contentType(MediaType.APPLICATION_JSON)
        ).andDo(print())
        .andExpect(status().isUnauthorized());
  ...
  @Test
  @WithAnonymousUser
  void 좋아요버튼클릭_로그인하지_않은경우() throws Exception {
    mockMvc.perform(post("/api/v1/posts/1/likes")
            .contentType(MediaType.APPLICATION_JSON)
        ).andDo(print())
        .andExpect(status().isUnauthorized());
  ...
  @Test
  @WithMockUser
  void 좋아요버튼클시_게시글이_없는경우() throws Exception {
